...
val ParticleSwarmTest by testSuite {
    ...
    test("a particle can be updated") {
        Particle(
            Vector(3, 2, 1),
            Vector(2, 1, -1),
            Vector(-1, 1, 2)
    ...
    test("part 1: the particle that will stay closest to position <0,0,0> in the long term") {
        ParticleSwarm().part1(input) shouldBe 0
    }

    test("part 2: only one particle is left after all collisions are resolved") {
        val input = """
